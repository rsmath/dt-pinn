...
class Trainer:
    def __init__(self, config=None, **kwargs):
        ...
        self.z_interior = torch.vstack([self.z_i.clone()])
        self.X_interior = torch.hstack([self.x_interior, self.y_interior, self.z_interior])

        # interior and boundary points
        self.x_tilde = torch.vstack([self.x_i.clone(), self.x_b.clone()])
        self.y_tilde = torch.vstack([self.y_i.clone(), self.y_b.clone()])
        self.z_tilde = torch.vstack([self.z_i.clone(), self.z_b.clone()])
        self.X_tilde = torch.hstack([self.x_tilde, self.y_tilde, self.z_tilde])
        ...
        self.z_full = torch.vstack([self.z_i.clone(), self.z_b.clone(), self.z_g.clone()])
        self.X_full = torch.hstack([self.x_full, self.y_full, self.z_full])

        # boundary points
        self.X_b = torch.hstack([self.x_b.clone(), self.y_b.clone(), self.z_b.clone()])
    ...
    def train(self):
        ...
        for i in range(1, epochs + 1):
            try:
                ...
                u_z = grad(pred_tilde, self.z_tilde, grad_outputs=torch.ones_like(
                           pred_tilde), create_graph=True, retain_graph=True)[0]
                u_zz = grad(u_z, self.z_tilde, grad_outputs=torch.ones_like(
                            pred_tilde), create_graph=True, retain_graph=True)[0]

                # Poisson residual
                pde_residual_training = (u_xx + u_yy + u_zz) - self.f
                ...
                l2_w_y = grad(boundary_pred, self.y_b, grad_outputs=torch.ones_like(boundary_pred),
                              create_graph=True, retain_graph=True)[0]
                l2_w_z = grad(boundary_pred, self.z_b, grad_outputs=torch.ones_like(boundary_pred),
                              create_graph=True, retain_graph=True)[0]

                # combining the partial derivatives
                w_xyz = torch.hstack([l2_w_x, l2_w_y, l2_w_z])

                # element wise dot product between the normal vectors and first partial derivatives
                gradient_n = torch.multiply(self.n, w_xyz).sum(dim=1).unsqueeze(dim=1)
                ...
                u_z = grad(pred_tilde_test, self.test_z_tilde, grad_outputs=torch.ones_like(
                           pred_tilde_test), create_graph=True, retain_graph=True)[0]
                u_zz = grad(u_z, self.test_z_tilde, grad_outputs=torch.ones_like(
                            pred_tilde_test), create_graph=True, retain_graph=True)[0]

                # Poisson residual
                pde_residual_test = (u_xx + u_yy + u_zz) - self.f_test
                ...
                l2_w_y = grad(boundary_pred, self.test_y_b, grad_outputs=torch.ones_like(boundary_pred),
                              create_graph=True, retain_graph=True)[0]
                l2_w_z = grad(boundary_pred, self.test_z_b, grad_outputs=torch.ones_like(boundary_pred),
                              create_graph=True, retain_graph=True)[0]
                ...
                # element wise dot product between the normal vectors and first partial derivatives
                gradient_n = torch.multiply(self.n_test, w_xyz).sum(dim=1).unsqueeze(dim=1)
...
if __name__ == "__main__":
    ...
    orders = [2, 3, 4, 5]
    training_size = [810, 3080, 4325, 6218]
    supervised_options = [False]
    activation_function = 'tanh'
    lr = 0.0025

    for network_precision in network_precisions:
        results_folder = f"gpu_3d_{network_precision}_cupy_csr_results_noprint"
        ...
        for cur_supervised_option in supervised_options:
            ...
            for order in orders:
                for size in training_size:
                    ...
                    print('\n\n')
                    print('+'*70)
                    print(f'Running 3D cupy {network_precision} approach on {file_name} with {activation_function}')
                    print('+'*70)
                    print('\n\n')

                    # read mat files
                    X_i = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/Xi.mat")["Xi"], dtype=PRECISION, requires_grad=True).to(device_string)
                    X_b = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/Xb.mat")["Xb"], dtype=PRECISION, requires_grad=True).to(device_string)
                    X_g = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/Xg.mat")["X_g"], dtype=PRECISION, requires_grad=True).to(device_string)
                    n = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/n.mat")["n"], dtype=PRECISION, requires_grad=True).to(device_string)
                    u_true = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/u.mat")["u"], dtype=PRECISION).to(device_string)
                    f = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/f.mat")["f"], dtype=PRECISION, requires_grad=True).to(device_string)
                    g = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/g.mat")["g"], dtype=PRECISION, requires_grad=True).to(device_string)
                    alpha = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/alpha.mat")["Neucoeff"], dtype=PRECISION, requires_grad=True).to(device_string)
                    beta = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/beta.mat")["Dircoeff"], dtype=PRECISION, requires_grad=True).to(device_string)
                    L = load_mat_cupy(loadmat(f"../scai_3d/files_{file_name}/L1.mat")["L1"])
                    B = load_mat_cupy(loadmat(f"../scai_3d/files_{file_name}/B1.mat")["B1"])
                    time.sleep(1)
                    b_starts = X_i.shape[0]
                    b_end = b_starts + X_b.shape[0]

                    # test files
                    X_i_test = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/Xi.mat")["Xi"], dtype=PRECISION, requires_grad=True).to(device_string)
                    X_b_test = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/Xb.mat")["Xb"], dtype=PRECISION, requires_grad=True).to(device_string)
                    test_u_true = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/u.mat")["u"], dtype=PRECISION).to(device_string)
                    f_test = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/f.mat")["f"], dtype=PRECISION, requires_grad=True).to(device_string)
                    g_test = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/g.mat")["g"], dtype=PRECISION, requires_grad=True).to(device_string)
                    alpha_test = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/alpha.mat")["Neucoeff"], dtype=PRECISION, requires_grad=True).to(device_string)
                    beta_test = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/beta.mat")["Dircoeff"], dtype=PRECISION, requires_grad=True).to(device_string)
                    n_test = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/n.mat")["n"], dtype=PRECISION, requires_grad=True).to(device_string)

                    test_x_i = X_i_test[:, 0].unsqueeze(dim=1)
                    test_y_i = X_i_test[:, 1].unsqueeze(dim=1)
                    test_z_i = X_i_test[:, 2].unsqueeze(dim=1)
                    test_x_b = X_b_test[:, 0].unsqueeze(dim=1)
                    test_y_b = X_b_test[:, 1].unsqueeze(dim=1)
                    test_z_b = X_b_test[:, 2].unsqueeze(dim=1)

                    test_x_tilde = torch.vstack([test_x_i, test_x_b])
                    test_y_tilde = torch.vstack([test_y_i, test_y_b])
                    test_z_tilde = torch.vstack([test_z_i, test_z_b])
                    test_X_tilde = torch.hstack([test_x_tilde, test_y_tilde, test_z_tilde])

                    test_X_b = torch.hstack([test_x_b, test_y_b, test_z_b])
