        ...
        for i in range(1, epochs + 1):
            try:
                def closure():
                    ...
                    u_z = grad(u_pred_interior, self.z_interior, grad_outputs=torch.ones_like(
                               u_pred_interior), create_graph=True, retain_graph=True)[0]
                    u_zz = grad(u_z, self.z_interior, grad_outputs=torch.ones_like(
                                u_pred_interior), create_graph=True, retain_graph=True)[0]

                    # Poisson residual
                    f_pred = (u_xx + u_yy + u_zz) - self.f_interior
...
if __name__ == "__main__":
    ...
    for save_device_string in device_strings:
        ...
        for precision_string in precision_strings:
            ...
            results_folder = f"{save_device_string}_3d_{precision_string}_vanilla_results_noprint"
            for cur_supervised_option in supervised_options:
                ...
                for order in orders:
                    for size in training_size:
                        ...
                        print('\n\n')
                        print('+'*70)
                        print(f'Running {precision_string} 3D vanilla-PINN on {file_name} with {activation_function}')
                        print('+'*70)
                        print('\n\n')

                        # read mat files
                        X_i = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/Xi.mat")["Xi"], dtype=PRECISION, requires_grad=True).to(device)
                        X_b = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/Xb.mat")["Xb"], dtype=PRECISION, requires_grad=True).to(device)
                        X_g = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/Xg.mat")["X_g"], dtype=PRECISION, requires_grad=True).to(device)
                        n = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/n.mat")["n"], dtype=PRECISION, requires_grad=True).to(device)
                        u_true = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/u.mat")["u"], dtype=PRECISION).to(device)
                        f = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/f.mat")["f"], dtype=PRECISION, requires_grad=True).to(device)
                        g = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/g.mat")["g"], dtype=PRECISION, requires_grad=True).to(device)
                        alpha = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/alpha.mat")["Neucoeff"], dtype=PRECISION, requires_grad=True).to(device)
                        beta = torch.tensor(loadmat(f"../scai_3d/files_{file_name}/beta.mat")["Dircoeff"], dtype=PRECISION).to(device)
                        b_starts = X_i.shape[0]
                        b_end = b_starts + X_b.shape[0]


                        # test files
                        X_i_test = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/Xi.mat")["Xi"], dtype=PRECISION, requires_grad=True).to(device)
                        X_b_test = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/Xb.mat")["Xb"], dtype=PRECISION, requires_grad=True).to(device)
                        test_u_true = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/u.mat")["u"], dtype=PRECISION).to(device)
                        f_test = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/f.mat")["f"], dtype=PRECISION, requires_grad=True).to(device)
                        g_test = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/g.mat")["g"], dtype=PRECISION, requires_grad=True).to(device)
                        alpha_test = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/alpha.mat")["Neucoeff"], dtype=PRECISION, requires_grad=True).to(device)
                        beta_test = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/beta.mat")["Dircoeff"], dtype=PRECISION).to(device)
                        n_test = torch.tensor(loadmat(f"../scai_3d/files_{test_name}/n.mat")["n"], dtype=PRECISION, requires_grad=True).to(device)
