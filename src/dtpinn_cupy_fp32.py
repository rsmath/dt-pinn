...
class Trainer:
    ...
    def train(self):
        ...
        rand_vec = cupy.from_dlpack(to_dlpack(torch.rand(self.L.shape[1], 2).to(torch.float32).to(device_string)))
        self.L.dot(rand_vec)
        self.B.dot(rand_vec)
        
        L_t = csr_matrix(self.L.transpose().astype(np.float32))
        B_t = csr_matrix(self.B.transpose().astype(np.float32))
        
        # initializing the matvec kernel for the sparse matrices' transposes
        rand_L_vec = cupy.from_dlpack(to_dlpack(torch.rand(self.L.shape[0], 2).to(torch.float32).to(device_string)))
        rand_B_vec = cupy.from_dlpack(to_dlpack(torch.rand(self.B.shape[0], 2).to(torch.float32).to(device_string)))
        L_t.dot(rand_L_vec)
        B_t.dot(rand_B_vec)
...
def load_mat_cupy(mat):
    csr = csr_matrix(mat.astype(np.float32))
    return csr
...
if __name__ == "__main__":
    ...
    for network_precision in network_precisions:
        results_folder = f"gpu_discrete_cupy_csr_results_{network_precision}network_noprint"
