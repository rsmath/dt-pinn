...
if __name__ == "__main__":
    ...
    for network_precision in network_precisions:
        results_folder = f"gpu_noisy_{network_precision}_cupy_csr_results_noprint"
        ...
        for cur_supervised_option in supervised_options:
            ...
            for order in orders:
                for alpha_value in alphas:
                    ...
                    save_folder = f'../{results_folder}/discrete_pinn/{order}/{size}/{alpha_value}/{supervised_file_bool}/'
                    ...
                    print('\n\n')
                    print('+'*70)
                    print(f'Running noisy cupy {network_precision} approach on {file_name} with {activation_function}')
                    print('+'*70)
                    print('\n\n')
                    ...
                    f = torch.tensor(loadmat(f"../scai/files_{file_name}/f_{alpha_value}.mat")["f"], dtype=PRECISION, requires_grad=True).to(device_string)
                    g = torch.tensor(loadmat(f"../scai/files_{file_name}/g_{alpha_value}.mat")["g"], dtype=PRECISION, requires_grad=True).to(device_string)
