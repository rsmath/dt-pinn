...
if __name__ == "__main__":
    ...
    for precision_string in precision_strings:
        ...
        results_folder = f"gpu_nonlinear_data_{precision_string}_vanilla_results_noprint"
        for cur_supervised_option in supervised_options:
            ...
            for order in orders:
                for size in training_size:
                    ...
                    print('\n\n')
                    print('+'*70)
                    print(f'Running {precision_string} nonlinear_data vanilla-PINN on {file_name} with {activation_function}')
                    print('+'*70)
                    print('\n\n')

                    # read mat files
                    X_i = torch.tensor(loadmat(f"../nonlinear/files_{file_name}/Xi.mat")["Xi"], dtype=PRECISION, requires_grad=True).to(device)
                    X_b = torch.tensor(loadmat(f"../nonlinear/files_{file_name}/Xb.mat")["Xb"], dtype=PRECISION, requires_grad=True).to(device)
                    X_g = torch.tensor(loadmat(f"../nonlinear/files_{file_name}/Xg.mat")["X_g"], dtype=PRECISION, requires_grad=True).to(device)
                    n = torch.tensor(loadmat(f"../nonlinear/files_{file_name}/n.mat")["n"], dtype=PRECISION, requires_grad=True).to(device)
                    u_true = torch.tensor(loadmat(f"../nonlinear/files_{file_name}/u.mat")["u"], dtype=PRECISION).to(device)
                    f = torch.tensor(loadmat(f"../nonlinear/files_{file_name}/f.mat")["f"], dtype=PRECISION, requires_grad=True).to(device)
                    g = torch.tensor(loadmat(f"../nonlinear/files_{file_name}/g.mat")["g"], dtype=PRECISION, requires_grad=True).to(device)
                    alpha = torch.tensor(loadmat(f"../nonlinear/files_{file_name}/alpha.mat")["Neucoeff"], dtype=PRECISION, requires_grad=True).to(device)
                    beta = torch.tensor(loadmat(f"../nonlinear/files_{file_name}/beta.mat")["Dircoeff"], dtype=PRECISION, requires_grad=True).to(device)
                    b_starts = X_i.shape[0]
                    b_end = b_starts + X_b.shape[0]


                    # test files
                    X_i_test = torch.tensor(loadmat(f"../nonlinear/files_{test_name}/Xi.mat")["Xi"], dtype=PRECISION, requires_grad=True).to(device)
                    X_b_test = torch.tensor(loadmat(f"../nonlinear/files_{test_name}/Xb.mat")["Xb"], dtype=PRECISION, requires_grad=True).to(device)
                    test_u_true = torch.tensor(loadmat(f"../nonlinear/files_{test_name}/u.mat")["u"], dtype=PRECISION).to(device)
                    f_test = torch.tensor(loadmat(f"../nonlinear/files_{test_name}/f.mat")["f"], dtype=PRECISION, requires_grad=True).to(device)
                    g_test = torch.tensor(loadmat(f"../nonlinear/files_{test_name}/g.mat")["g"], dtype=PRECISION, requires_grad=True).to(device)
                    alpha_test = torch.tensor(loadmat(f"../nonlinear/files_{test_name}/alpha.mat")["Neucoeff"], dtype=PRECISION, requires_grad=True).to(device)
                    beta_test = torch.tensor(loadmat(f"../nonlinear/files_{test_name}/beta.mat")["Dircoeff"], dtype=PRECISION, requires_grad=True).to(device)
                    n_test = torch.tensor(loadmat(f"../nonlinear/files_{test_name}/n.mat")["n"], dtype=PRECISION, requires_grad=True).to(device)
